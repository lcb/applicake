...
class mProphet(IWrapper):
    ...
    def prepare_run(self, info, log):
        ...
        command = 'mProphetScoreSelector.sh %s %s %s && mProphetRunner.sh %s %s' % (
        info['FEATURETSV'], info['MPR_MAINVAR'], info['MPR_VARS'], mod_template, lda)
        return command, info
    ...
    def validate_run(self, info, log, run_code, out_stream, err_stream):
        ...
        resultfile = os.path.join(info[Keys.WORKDIR], self._projectname + '_all_peakgroups.xls')
        ...
        for statfile in ['.pdf', '_raw_stat.xls', '_stat.xls']:
            fullpath = os.path.join(info[Keys.WORKDIR], self._projectname + statfile)
        ...
        if info['MPR_LDA_PATH'] != '':
            ...
            classifier = os.path.join(info[Keys.WORKDIR], self._projectname + '_classifier.xls')
