...
class OpenSwathAnalyzerParallel(IWrapper):
    '''
    Wrapper for the Analyzer in OpenSWATH.
    '''
    _outfiles = []
    def get_template_handler(self):
        return OpenSwathAnalyzerParallelTemplate()
    ...
    def prepare_run(self,info,log):
        ...
        if info['WINDOW_UNIT'] == 'ppm':
            ppm = '-ppm'
        ceopts = '-extraction_window %s %s -rt_extraction_window %s -tr %s -min_upper_edge_dist %s -rt_norm %s -is_swath' % ( 
        ...
        info['TEMPLATE'] = os.path.join(info['WORKDIR'],'template.ini')
        _,info = self.get_template_handler().modify_template(info, log)
        
        anopts = '-tr %s -min_upper_edge_dist %s -rt_norm %s -ini %s -no-strict' % (
                      info['TRAML'],
                      info['MIN_UPPER_EDGE_DIST'],
                      info['TRAFOXML'],
                      info[self.TEMPLATE])
        
        convopts = "-tr %s -short_format" %(info['TRAML'])
        mzmldir = os.path.dirname(info['MZML'][0])
        
        
         
        command = 'OpenSwathAnalyzerParallel.sh -c "%s" -a "%s" -f "%s" -m %s -t %s -o %s %s' % (ceopts,anopts,convopts,mzmldir,info[self.THREADS],info[self.WORKDIR],' '.join(info['MZML']))
...
class OpenSwathAnalyzerParallelTemplate(BasicTemplateHandler):
    """
