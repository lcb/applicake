...
class OpenSwathAnalyzerToTSV(IWrapper):
    ...
    def prepare_run(self, info, log):
        ...
        (" ".join(info['CHROM_MZML']),
         ...
         info['TRAML'],info['MIN_UPPER_EDGE_DIST'],info['TRAFOXML'],info[Keys.TEMPLATE], info[Keys.WORKDIR],
         info['TRAML'], info[Keys.WORKDIR], info[Keys.WORKDIR],
         info[Keys.THREADS],
         info[Keys.WORKDIR], outfile)

        info['FEATURETSV'] = outfile
        return command, info
...
class AnalyzerTemplate(BasicTemplateHandler):
