...
class Dss(IWrapper):
    ...
    def prepare_run(self, info, log):
        ...
        if len(self.outkeys) == 0:
            self.outkeys = [Keys.DSSOUTPUT]
            try:
                self.outkeys.append(Dss.DEFAULT_KEYS[prefix])
            except:
                pass

        if info[Keys.PREFIX] == 'getexperiment' and info.has_key(self.EXP_CODE):
            dscode_key = self.EXP_CODE
        else:
            dscode_key = Keys.DATASET_CODE
        if not info.has_key(dscode_key):
            log.error("%s (or %s) must be set in ini file" % (Keys.DATASET_CODE, self.EXP_CODE))
            sys.exit(3)
        ...
        if type(dataset_codes) is str:
            ...
            log.error("%s (%s) must be either str or list" % (dscode_key, info[dscode_key]))
            sys.exit(4)

        outdir = info[Keys.DATASET_DIR]
        if not os.path.isdir(outdir):
            if os.system("test -d %s" % outdir):
                log.error("value [%s] of key [%s] is not a directory" % (outdir, Keys.DATASET_DIR))
                sys.exit(5)
        ...
        #print prefix,outdir,self._codes
    ...
    def set_args(self, log, args_handler):
        ...
        See interface
        """
        args_handler.add_app_args(log, Keys.DATASET_DIR, 'download directory')
        args_handler.add_app_args(log, Keys.DATASET_CODE, 'data set code')
        args_handler.add_app_args(log, self.EXP_CODE,
                                  'experiment code, can be used for getexperiment in lieu of %s (and to distinguish from the latter)' % Keys.DATASET_CODE)
        args_handler.add_app_args(log, Keys.DSSKEYS, "controls the output. results are values of these keys. " +
                                                     "the default is 'DSSOUTPUT' plus potentially a prefix depending key",
                                  default='')
        args_handler.add_app_args(log, Keys.PREFIX, "one of these executables: %s" % self.ALLOWED_PREFIXES,
                                  choices=self.ALLOWED_PREFIXES)
        args_handler.add_app_args(log, 'RESULT_FILE',
                                  "result file of the dss-client call, contains pathes to downloaded files. " +
        ...
        args_handler.add_app_args(log, 'KEEP_NAME',
                                  "for prefix is 'gegmsdata' only: output keeps original file name if set to true " +
                                  "(otherwise it will be changed to samplecode~dscode.mzXXML)", default='False')
        args_handler.add_app_args(log, 'FAILURE_TOLERANT',
                                  "exits with 0 in cases where no data set was found if to true", default='False')
        return args_handler
