...
class Copy2IdentDropbox(Copy2Dropbox):
    ...
    def set_args(self,log,args_handler):
        args_handler = super(Copy2IdentDropbox,self).set_args(log,args_handler)
        ...
        args_handler.add_app_args(log, 'WORKFLOW', 'Workflow name')
        args_handler.add_app_args(log, 'FRAGMASSERR', 'Fragment mass error')
        args_handler.add_app_args(log, 'FRAGMASSUNIT', 'Fragment mass unit')
        args_handler.add_app_args(log, 'PRECMASSERR', 'Precursor mass error')
        args_handler.add_app_args(log, 'PRECMASSUNIT', 'Precursor mass unit')
        args_handler.add_app_args(log, 'MISSEDCLEAVAGE', 'Missed cleavages')                  
        return args_handler            