...
class OpenMs(IWrapper):
    ...
    def set_args(self, log, args_handler):
        ...
        """
        args_handler.add_app_args(log, Keys.PREFIX, 'Path to the OpenMS executable')
        args_handler.add_app_args(log, Keys.TEMPLATE, 'Path to the openMS-template file')
        args_handler.add_app_args(log, Keys.THREADS, 'Number of threads used in the process.')
        args_handler.add_app_args(log, Keys.WORKDIR, 'Directory to store files')
        return args_handler
...
class SearchEngineAdapter(OpenMs):
    ...
    def set_args(self, log, args_handler):
        ...
        args_handler.add_app_args(log, self.VARIABLE_MODS, 'List of variable modifications',
                                  choices=ModificationDb().get_keys())
        args_handler.add_app_args(log, Keys.MZML, 'Path to the mzML file.')
        return args_handler
