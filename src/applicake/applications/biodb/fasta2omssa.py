...
class Fasta2Omssa(IWrapper):
    '''
    Wrapper for formatdb a program from the BLAST package and needed to create binaries for OMSSA.
    '''

    _template_file = ''
    _result_file = ''
    _default_prefix = 'formatdb'
...
#        info[key] = self._result_file
        app_log = os.path.join(wd,'formatdb.log')
        ...
        command = '%s -i %s %s -l %s'% (prefix,info[self.FASTA],mod_template, app_log)
...
class Fasta2OmssaTemplate(BasicTemplateHandler):
    """
    Template handler for Fasta2Omssa.
    """
