...
class Mimic(IWrapper):
    '''
    Wrapper for mimic from the percolator package.
    '''

    _template_file = ''
    _result_file = ''
    _default_prefix = 'mimic'
    ...
    def prepare_run(self,info,log):
        ...
        fasta = info[self.FASTA]
        fasta_decoy = []
        commands = []
        prefix,info = self.get_prefix(info,log)
        for i,e in enumerate(fasta):
            basename = os.path.basename(e)
            fasta_decoy.append(os.path.join(wd,basename))
            # copy original fasta file to new location
            # append decoy output to copy
            commands.append('%s %s > %s' % (prefix,fasta[i],fasta_decoy[i]))
        command = ';'.join(commands)
        info[self.FASTA_DECOY] = fasta_decoy
        return command,info
