...
def generator(input_file_name, notused_output_file_names):
    submitter.run(lsfargs,'run_guse_generator.py',['-i', input_file_name, '--GENERATORS', 'generate.ini'])
    
    
@transform(generator, regex("generate.ini_"), "dss.ini_")
def dss(input_file_name, output_file_name):   
    submitter.run(lsfargs,'run_dss.py', ['-i',  input_file_name,'-o', output_file_name,'--PREFIX', 'getmsdata'])

@transform(dss, regex("dss.ini_"), "xtandem.ini_")
def tandem(input_file_name, output_file_name):
    submitter.run(lsfargs,'run_xtandem.py', ['-i',  input_file_name,'-o', output_file_name,'--PREFIX', 'tandem.exe'])

@transform(tandem, regex("xtandem.ini_"), "xtandem2xml.ini_")
def tandem2xml(input_file_name, output_file_name):
    submitter.run(lsfargs,'run_tandem2xml.py', ['-i',  input_file_name,'-o', output_file_name])  

@transform(tandem2xml, regex("xtandem2xml.ini_"), "xinteract.ini_")
def xinteract(input_file_name, output_file_name):
    submitter.run(lsfargs,'run_xinteract.py', ['-i',  input_file_name,'-o', output_file_name])  

@merge(xinteract, "collector.ini")
def collector(notused_input_file_names, output_file_name):
    submitter.run(lsfargs,'run_guse_collector.py', ['--COLLECTORS', 'xinteract.ini' ,'-o', output_file_name])    

@follows(collector)
def unifier():
    submitter.run(lsfargs,'run_unify.py', ['-i', 'collector.ini','-o', 'unifier.ini' , '--UNIFIER_REDUCE'])  
