...
def generator(input_file_name, notused_output_file_names):
    DynRunner.run(
        ['applicake.applications.commons.generator.IniDatasetcodeGenerator', '-i', input_file_name, '--GENERATOR',
         'generate.ini'])


@transform(generator, regex("generate.ini_"), "echoout.ini_")
def echo(input_file_name, output_file_name):
    DynRunner.run(['applicake.applications.os.echo.Echo', '-i', input_file_name, '-o', output_file_name])


@merge(echo, "collector.ini")
def collector(notused_input_file_names, output_file_name):
    DynRunner.run(
        ['applicake.applications.commons.collector.IniCollector', '-i', 'echoout.ini_0', '--COLLECTOR', 'echoout.ini',
         '-o', output_file_name])
...
def paramgenerate(input_file_name, output_file_name):
    DynRunner.run(
        ['applicake.applications.commons.generator.IniParametersetGenerator', '-i', "collector.ini", '--GENERATOR',
         'paramgenerate.ini'])
